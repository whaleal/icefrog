...
import com.whaleal.icefrog.db.sql.*;
...
public class DialectRunner implements Serializable {
    ...
    public <T> T find( Connection conn, Query query, RsHandler<T> rsh ) throws SQLException {
        checkConn(conn);
        Precondition.notNull(query, "[query] is null !");
    ...
    /**
     * 获取查询结果总数，生成类似于 SELECT count(1) from (sql) icefrog_alias_count_<br>
    ...
    private void checkConn( Connection conn ) {
        Precondition.notNull(conn, "Connection object must be not null!");
    }
    //---------------------------------------------------------------------------- Private method start
}
