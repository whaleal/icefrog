...
public class HttpUtilTest {
	...
	public void getTest3() {
		// 测试url中带有空格的情况
		String result1 = HttpUtil.get("http://icefrog.cn:5000/kf?abc= d");
		Console.log(result1);
	}
	...
	public void encodeParamTest() {
		...
		// URL原样输出
		paramsStr = "https://www.icefrog.cn/";
		encode = HttpUtil.encodeParams(paramsStr, CharsetUtil.CHARSET_UTF_8);
		...
		paramsStr = "https://www.icefrog.cn/?";
		encode = HttpUtil.encodeParams(paramsStr, CharsetUtil.CHARSET_UTF_8);
		Assert.assertEquals("https://www.icefrog.cn/", encode);
	}
	...
	public void urlWithFormTest() {
		...
		String urlWithForm = HttpUtil.urlWithForm("http://api.icefrog.cn/login?type=aaa", param, CharsetUtil.CHARSET_UTF_8, false);
		Assert.assertEquals(
				"http://api.icefrog.cn/login?type=aaa&AccessKeyId=123&Action=DescribeDomainRecords&Format=date&DomainName=lesper.cn&SignatureMethod=POST&SignatureNonce=123&SignatureVersion=4.3.1&Timestamp=123432453&Version=1.0",
				urlWithForm);

		urlWithForm = HttpUtil.urlWithForm("http://api.icefrog.cn/login?type=aaa", param, CharsetUtil.CHARSET_UTF_8, false);
		Assert.assertEquals(
				"http://api.icefrog.cn/login?type=aaa&AccessKeyId=123&Action=DescribeDomainRecords&Format=date&DomainName=lesper.cn&SignatureMethod=POST&SignatureNonce=123&SignatureVersion=4.3.1&Timestamp=123432453&Version=1.0",
				urlWithForm);
	}
