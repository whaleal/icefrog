...
public class ArrayTableTest extends AbstractTableTest {
  ...
  public void testHashCode() {
    ...
    table.put("foo", 3, 'c');
    int expected =
        ObjectUtil.hashCode("foo", 1, 'a')
            + ObjectUtil.hashCode("bar", 1, 'b')
            + ObjectUtil.hashCode("foo", 3, 'c')
            + ObjectUtil.hashCode("bar", 3, 0);
  ...
  public void testCellReflectsChanges() {
    table = create("foo", 1, 'a', "bar", 1, 'b', "foo", 3, 'c');
    Table.Cell<String, Integer, Character> cell = table.cellSet().iterator().next();
