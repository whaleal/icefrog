 ...
 *
 * <p>If there are no removals, then iteration order for the {@link #entrySet}, , and
...
class CompactHashMap<K extends Object, V extends Object>
   ...
   * its bottom  bits, with a modification count in
    ...
    if (next == UNSET) { // uninitialized bucket
      ...
      do {
        ...
        if (CompactHashing.getHashPrefix(entry, mask) == hashPrefix
            && ObjectUtil.equal(key, keys[entryIndex])) {
  ...
  private int indexOf(@CheckForNull Object key) {
    ...
    do {
      ...
      if (CompactHashing.getHashPrefix(entry, mask) == hashPrefix
          && ObjectUtil.equal(key, key(entryIndex))) {
        return entryIndex;
      }
      next = CompactHashing.getNext(entry, mask);
    } while (next != UNSET);
  ...
  void moveLastEntry(int dstIndex, int mask) {
    ...
    if (dstIndex < srcIndex) {
      ...
      if (next == srcNext) {
        ...
        // here, entries[entryIndex] poNumberUtil to the old entry location; update it
  ...
  class EntrySetView extends Maps.EntrySet<K, V> {
    ...
    public boolean contains(@CheckForNull Object o) {
      ...
      if (delegate != null) {
        ...
        return index != -1 && ObjectUtil.equal(value(index), entry.getValue());
      }
      return false;
    }
  ...
  final class MapEntry extends AbstractMapEntry<K, V> {
    ...
    private void updateLastKnownIndex() {
      if (lastKnownIndex == -1
          || lastKnownIndex >= size()
          || !ObjectUtil.equal(key, key(lastKnownIndex))) {
    ...
    public V setValue(@ParametricNullness V value) {
      Map<K, V> delegate = delegateOrNull();
      if (delegate != null) {
        return (delegate.put(key, value)); // See discussion in getValue().
      }
      updateLastKnownIndex();
      if (lastKnownIndex == -1) {
        put(key, value);
  ...
  public boolean containsValue(@CheckForNull Object value) {
    ...
    for (int i = 0; i < size; i++) {
      if (ObjectUtil.equal(value, value(i))) {
        return true;
      }
    }
    return false;
  ...
  public void clear() {
    ...
    if (delegate != null) {
      metadata =
          NumberUtil.constrainToRange(size(), CompactHashing.DEFAULT_SIZE, CompactHashing.MAX_SIZE);
