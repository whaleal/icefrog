...
class MapMakerInternalMap<
   ...
   * that entries are indexable using NumberUtil.
   */
  static final int MAXIMUM_CAPACITY = NumberUtil.MAX_POWER_OF_TWO;
  ...
  private MapMakerInternalMap(MapMaker builder, InternalEntryHelper<K, V, E, S> entryHelper) {
    ...
    // Find power-of-two sizes best matching arguments. ConstraNumberUtil:
   ...
   */
  static <K>
      MapMakerInternalMap<K, MapMaker.Dummy, ? extends InternalEntry<K, Dummy, ?>, ?> createWithDummyValues(
          MapMaker builder) {
    if (builder.getKeyStrength() == Strength.STRONG
        && builder.getValueStrength() == Strength.STRONG) {
      ...
      for (int oldIndex = 0; oldIndex < oldCapacity; ++oldIndex) {
        ...
        if (head != null) {
          ...
          if (next == null) {
            ...
            // index from the end of the list. tail poNumberUtil to the first
  ...
  public int size() {
    ...
    return  (int)NumberUtil.saturatedCast(sum ,Integer.class);
  }
