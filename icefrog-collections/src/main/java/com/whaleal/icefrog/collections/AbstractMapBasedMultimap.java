...
abstract class AbstractMapBasedMultimap<K extends Object, V extends Object>
  ...
  private class KeySet extends CKeySet<K, Collection<V>> {
  ...
  private void removeValuesForKey(@CheckForNull Object key) {
    Collection<V> collection = MapUtil.safeRemove(map, key);
  ...
  private abstract class Itr<T extends Object> implements Iterator<T> {
    ...
    Itr() {
      ...
      key = null;
      collection = null;
      valueIterator = IterUtil.emptyModifiableIterator();
    }
  ...
  Iterator<Entry<K, V>> entryIterator() {
    return new Itr<Entry<K, V>>() {
      ...
      Entry<K, V> output(@ParametricNullness K key, @ParametricNullness V value) {
        return MapUtil.immutableEntry(key, value);
      }
    };
  }
    ...
    return SpliteratorUtil.flatMap(
        ...
        keyToValueCollectionEntry -> {
          ...
          return SpliteratorUtil.map(
              valueCollection.spliterator(), (V value) -> MapUtil.immutableEntry(key, value));
        },
        Spliterator.SIZED,
        size());
  }
  ...
  private class AsMap extends ViewCachingAbstractMap<K, Collection<V>> {
    ...
    @Override
    public boolean containsKey(@CheckForNull Object key) {
      return MapUtil.safeContainsKey(submap, key);
    }

    @Override
    @CheckForNull
    public Collection<V> get(@CheckForNull Object key) {
      Collection<V> collection = MapUtil.safeGet(submap, key);
      if (collection == null) {
        return null;
      }
    ...
    public void clear() {
      if (submap == map) {
        ...
        IterUtil.clear(new AsMapIterator());
      }
    }

    Entry<K, Collection<V>> wrapEntry(Entry<K, Collection<V>> entry) {
      K key = entry.getKey();
      return MapUtil.immutableEntry(key, wrapCollection(key, entry.getValue()));
    }

   
    class AsMapEntries extends AbsEntrySet<K, Collection<V>> {
      @Override
      ...
      public Spliterator<Entry<K, Collection<V>>> spliterator() {
        return SpliteratorUtil.map(submap.entrySet().spliterator(), AsMap.this::wrapEntry);
      }
      ...
      @Override
      public boolean contains(@CheckForNull Object o) {
        return CollUtil.safeContains(submap.entrySet(), o);
      }
  ...
  class NavigableAsMap extends SortedAsMap implements NavigableMap<K, Collection<V>> {
    ...
    Entry<K, Collection<V>> pollAsMapEntry(Iterator<Entry<K, Collection<V>>> entryIterator) {
      ...
      entryIterator.remove();
      return MapUtil.immutableEntry(entry.getKey(), unmodifiableCollectionSubclass(output));
    }
