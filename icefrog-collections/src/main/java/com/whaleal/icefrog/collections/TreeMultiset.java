...
public final class TreeMultiset<E extends Object> extends AbstractSortedMultiset<E>
  ...
  private long aggregateBelowRange(Aggregate aggr, @CheckForNull AvlNode<E> node) {
    ...
    int cmp =
        comparator()
            .compare((range.getLowerEndpoint()), node.getElement());
    if (cmp < 0) {
  ...
  private long aggregateAboveRange(Aggregate aggr, @CheckForNull AvlNode<E> node) {
    ...
    int cmp =
        comparator()
            .compare((range.getUpperEndpoint()), node.getElement());
    if (cmp > 0) {
  ...
  @Override
  public int size() {
    return (int) NumberUtil.saturatedCast(aggregateForEntries(Aggregate.SIZE),Integer.class);
  }

  @Override
  int distinctElements() {
    return (int) NumberUtil.saturatedCast(aggregateForEntries(Aggregate.DISTINCT),Integer.class);
  }
  ...
  private static final class AvlNode<E extends Object> {
    ...
    private AvlNode<E> ceiling(Comparator<? super E> comparator, @ParametricNullness E e) {
      int cmp = comparator.compare(e, getElement());
      if (cmp < 0) {
        return (left == null) ? this : firstNonNull(left.ceiling(comparator, e), this);
      } else if (cmp == 0) {
        return this;
      } else {
    ...
    private AvlNode<E> floor(Comparator<? super E> comparator, @ParametricNullness E e) {
      int cmp = comparator.compare(e, getElement());
      if (cmp > 0) {
        return (right == null) ? this : firstNonNull(right.floor(comparator, e), this);
      } else if (cmp == 0) {
        return this;
      } else {
