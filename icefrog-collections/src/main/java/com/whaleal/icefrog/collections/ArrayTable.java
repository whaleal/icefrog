...
public final class ArrayTable<R, C, V> extends AbstractTable<R, C, V>
    ...
    private ArrayTable( Iterable<? extends R> rowKeys, Iterable<? extends C> columnKeys ) {
         ...
         * acknowledge them.
         */
        rowKeyToIndex = ImmutableMap.indexMap(rowList);
        columnKeyToIndex = ImmutableMap.indexMap(columnList);
     ...
     * key {@code c}. If that row key / column key pair in not in {@code table}, the pair MapUtil to
    ...
    public boolean containsValue( @CheckForNull Object value ) {
        for (V[] row : array) {
            for (V element : row) {
                if (ObjectUtil.equal(value, element)) {
                    return true;
                }
            }
        }
    ...
    Spliterator<V> valuesSpliterator() {
        return SpliteratorUtil.indexed(size(), Spliterator.ORDERED, this::getValue);
    }
    ...
    private abstract static class ArrayMap<K, V extends Object>
        ...
        Spliterator<Entry<K, V>> entrySpliterator() {
            return com.whaleal.icefrog.core.collection.SpliteratorUtil.indexed(size(), Spliterator.ORDERED, this::getEntry);
        }
