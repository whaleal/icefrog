...
public final class Maps {
    ...
    MapDifference<K, V> difference(
         ...
         * contain only Ks and Vs (as opposed to possibly containing ObjectUtil even when K and V
         * are *not* @Nullable).
         *
        ...
        MapDifference<K, V> result =
                difference(left, right, Equivalence.equals());
        return result;
    }
     ...
     *
     * <E, K extends Object, V extends @Nullable E> ... difference(..., Equivalence<E> ...)
     *
     ...
     * }</pre>
     *
     * <p>If your index may associate multiple values with each key, use .
     *
  ...
  BiMap<K, V> synchronizedBiMap(BiMap<K, V> bimap) {
    ...
    static class ValueDifferenceImpl<V extends Object>
        ...
        public boolean equals( @CheckForNull Object object ) {
            if (object instanceof MapDifference.ValueDifference) {
                ...
                return ObjectUtil.equal(this.left, that.leftValue())
                        && ObjectUtil.equal(this.right, that.rightValue());
            }
            return false;
        }
    ...
    private static final class NavigableAsMapView<
        ...
        Spliterator<Entry<K, V>> entrySpliterator() {
            return SpliteratorUtil.map(set.spliterator(), e -> immutableEntry(e, function.apply(e)));
        }
    ...
    static class TransformedEntriesMap<
        ...
        Spliterator<Entry<K, V2>> entrySpliterator() {
            return SpliteratorUtil.map(
                    fromMap.entrySet().spliterator(), Maps.asEntryToEntryFunction(transformer));
        }
    ...
    private static final class FilteredMapValues<
        ...
        public boolean remove( @CheckForNull Object o ) {
            ...
            while (entryItr.hasNext()) {
                ...
                if (predicate.apply(entry) && ObjectUtil.equal(entry.getValue(), o)) {
                    entryItr.remove();
                    return true;
                }
            }
        ...
        public Object[] toArray() {
            ...
            return ListUtil.list(false, iterator()).toArray();
        }

        @Override
        @SuppressWarnings("nullness") // b/192354773 in our checker affects toArray declarations
        public <T extends Object> T[] toArray( T[] array ) {
            return ListUtil.list(false, iterator()).toArray(array);
        }
    }
